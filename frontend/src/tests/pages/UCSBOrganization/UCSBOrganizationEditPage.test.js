...
jest.mock('react-router-dom', () => {
    ...
    return {
        ...
        ...originalModule,
        useParams: () => ({
            id: "ZPR"  // Update to use 'id' instead of 'orgCode'
        }),
...
describe("UCSBOrganizationEditPage tests", () => {
    ...
    describe("tests where backend is working normally", () => {
        ...
        test("Is populated with the data provided", async () => {
            ...
            //fireEvent.change(orgCodeField, { target: { value: { id: 'ZZZ' } } });
            fireEvent.change(orgCodeField, { target: { value: 'ZZZ' } });
            fireEvent.change(orgTranslationShortField, { target: { value: 'ZP' } });
            fireEvent.change(orgTranslationField, { target: { value: 'ZPRS' } });
            fireEvent.change(inactiveField, { target: { value: 'false' } });
            fireEvent.click(submitButton);

            await waitFor(() => expect(mockToast).toBeCalled());
            expect(mockToast).toBeCalledWith("UCSBOrganization Updated - orgCode: ZPR orgTranslationShort: ZPR orgTranslation: ZETAS inactive: false");
            
            expect(mockNavigate).toBeCalledWith({ "to": "/ucsborganization" });

            expect(axiosMock.history.put.length).toBe(1); // times called
            
            console.log("Received orgCode:", axiosMock.history.put[0].params.orgCode);
            expect(axiosMock.history.put[0].params.orgCode).toEqual("ZZZ");
        ...
        test("Changes when you click Update", async () => {
            ...
            fireEvent.change(orgCodeField, { target: { value: { id: 'Z' } } });
            fireEvent.change(orgTranslationShortField, { target: { value: 'ZP' } });
