...
describe("UserTable tests", () => {
  ...
  test("Has the expected colum headers and content for adminUser", () => {
    ...
    const expectedHeaders = ["id", "ItemId","Stars", "DateReviewed", "Comments"];
    const expectedFields = ["id", "itemId", "stars", "dateReviewed", "comments"];
    const testId = "MenuItemReviewsTable";
  ...
  test("Delete button calls delete callback", async () => {
    ...
    expect(await screen.findByTestId(`MenuItemReviewsTable-cell-row-0-col-id`)).toHaveTextContent("1");
    expect(screen.getByTestId(`MenuItemReviewsTable-cell-row-0-col-comments`)).toHaveTextContent("best of the best");

    const deleteButton = screen.getByTestId(`MenuItemReviewsTable-cell-row-0-col-Delete-button`);
    expect(deleteButton).toBeInTheDocument();
